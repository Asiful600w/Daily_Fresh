...
import { AdminAuthProvider, AdminUser } from '@/context/AdminAuthContext';
...
function AdminProtectedLayout({
    ...
    useEffect(() => {
        // Use a small delay for redirect to allow AuthContext to settle during tab switches/hydrations
        let redirectTimeout: NodeJS.Timeout;

        if (!isPublicAdminPage && !adminLoading && !adminUser) {
            console.log('[LAYOUT DEBUG] Potential missing session detected, waiting for settlement...');

            redirectTimeout = setTimeout(() => {
                // Re-check after 1 second
                if (!adminUser && !adminLoading) {
                    console.log('[LAYOUT DEBUG] Redirecting to login - Session truly missing');
                    signOutAdmin().then(() => {
                        router.replace('/admin/login');
                    });
                }
            }, 1500); // 1.5s buffer for tab switching/network flickers
        }
    ...
    if (!adminUser) {
        // Show redirecting state instead of white screen/null
        return (
            <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-900">
                <div className="flex flex-col items-center gap-4">
                    <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
...
export default function AdminLayoutShell({
    ...
    return (
        <AdminAuthProvider initialUser={initialUser}>
